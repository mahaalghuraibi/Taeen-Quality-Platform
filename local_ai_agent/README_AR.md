...
> **الحالة الحالية:** PPE جاهز (`no_mask`، `no_gloves`، `no_headcover`)، ونموذج البيئة منشور ويكشف `wet_floor`.
> نموذج الزي الرسمي اختياري وغير جاهز — أضف `uniform_yolo.pt` لتفعيل `improper_uniform`.
> نماذج الزي والبيئة اختيارية: إذا كان أي ملف غير موجود يستمر الوكيل دون تعطّل ويعرض تحذيراً.
> **دقة نموذج الأرضية المبللة محدودة** (دُرِّب على بيانات صغيرة) — أجرِ التحقق النهائي بصور حقيقية من كاميرات المطعم (راجع `docs/AI_VALIDATION_GUIDE_AR.md`).
